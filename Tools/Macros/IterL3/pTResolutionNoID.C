...
void pTResolutionNoID(TString inputfilename="/eos/uscms/store/user/bmahakud/ProductionHLTAN_LPC_IterL3HighStat/DYJetsToLL_M-50_TuneCUETP8M1_13TeV-madgraphMLM-pythia8/ProductionHLTAN_LPC_IterL3HighStat/181130_193653/0000/muonNtupleIterL3.root", std::string effmeasured="MC2018"){
...
//  int nentries = 10000000;// tree->GetEntries();
  ...
  for (Int_t eventNo=0; eventNo < nentries; eventNo++)     {
	...
	HLTMuonCand closestL3;
        for ( std::vector<HLTMuonCand>::const_iterator it = ev -> hltNoIDmuons.begin(); it != ev -> hltNoIDmuons.end(); ++it ) {
	...
	if (match){
		...
		if (ev -> muons.at(jmu).charge != closestL3.charge) std::cout << "diff " << closestL3.charge << std::endl;
